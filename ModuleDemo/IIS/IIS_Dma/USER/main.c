...
uint8_t Flag;
/********************************************************************************/
// IIS的DMA发送示例，日志通过串口1发送，波特率为115200，采样率为8K，16位数据，DMA发送
/********************************************************************************/
int main(void)
{
...
void NVIC_Configuration(void)
{
	...
	NVIC_Init(&NVIC_InitStructure);							  //初始化DMA1通道5中断
}
...
void IIS_Configuration(void)
{
	...
	I2S_InitStructure.I2S_AudioFreq = I2S_AudioFreq_8k;		  //音频采样率8K
	I2S_InitStructure.I2S_CPOL = I2S_CPOL_Low;				  //时钟极性，低电平
	I2S_InitStructure.I2S_MCLKOutput = I2S_MCLKOutput_Enable; //允许MCLK输出
...
void DMA_Configuration(void)
{
	...
	DMA_InitStructure.DMA_BufferSize = BufferSize;							// DMA的Buffer大小
	...
	DMA_InitStructure.DMA_Priority = DMA_Priority_VeryHigh;					// DMA的优先级为高
...
{
	if (DMA_GetITStatus(DMA1_IT_TC5) == SET) //检查DMA1通道5传输是否完成
	{
		DMA_ClearITPendingBit(DMA1_IT_TC5); //清除DMA1通道5传输完成标志
		DMA_ClearFlag(DMA1_FLAG_TC5);		//清除DMA1通道5传输完成标志
		DMA_Cmd(DMA1_Channel5, DISABLE);	//关闭DMA1通道5
		Flag = 1;							//标志位置1
	}
}
